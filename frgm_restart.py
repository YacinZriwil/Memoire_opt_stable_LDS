...
def optimize(Y, X, U, gamma = 0.0001, min_gamma = 0.0000000001, alpha1 = 1, max_iterations = 10000, max_time = 20):
    N                   = X.shape[0]
    ...
    while i < max_iterations and time.time() - start_time < max_time:
        ...
        Q = Orthogonal_manifold.retraction(Q1, -gamma * gradR_Q1)
        T = Schur_manifold.projection(T1,  -gamma * grad_T1)
        B = - gamma * grad_B1
        ...
        while objective_function(Q, T, B, Y, X, U) > loss2 and gamma1 >= min_gamma: # si loss <= loss2 => on a réussi à décroitre (donc on passe dans le if d'en dessous); 
            gamma1 = 2/3 * gamma1
            Q = Orthogonal_manifold.retraction(Q1, -gamma1 * gradR_Q1)
            T = Schur_manifold.projection(T1, T1 - gamma1 * grad_T1)
            B = B1 - gamma1 * grad_B1
    ...
    print(f"\nLoss finale : {loss:.2f}   et   ||Y - AX - BU|| = {err}\n========================================") 
