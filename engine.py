...
def learn_stable_soc(X,Y, **kwargs):
    ...
    while i:
        ...
        # on calcule : newZ = Z - gamma* grad(Z) avec Z = (S,O,C)
        Sn = Ys - S_grad * step
        On = Yo - O_grad * step
        Cn = Yc - C_grad * step
        
        # on calcule : Z(k) = Projection(newZ)
        Sn, On, Cn = soc.project_to_feasible(Sn, On, Cn, **kwargs)
        
        Sn_inv = numpy.linalg.inv(a=Sn)
        e_new = utilities.adjusted_frobenius_norm(X=Y - (Sn_inv @ On @ Cn @ Sn) @ X)
        ...
        while e_new > e_old and inneriter <= fgm_max_iter:
            ...
            try:
                ...
                break
            except (AssertionError, NameError): # car le prev_error n'a pas été défini avant
                    pass
        ...
        if ((e_old < 1e-6 * nA2) or ((i > 100) and (e100[current_i_min_100] - e100[current_i]) < 1e-8*e100[current_i_min_100])):
            break
        e_old = e_new
...
def learn_stable_soc_with_inputs(X,Y, U, **kwargs):
    ...
    alpha = kwargs.get('alpha', 0.5)
    step_size_factor = kwargs.get('step_size_factor', 5)
    fgm_max_iter = kwargs.get('fgm_max_iter', 1000)
    conjugate_gradient = kwargs.get('conjugate_gradient', False)
    log_memory = kwargs.get('log_memory', True)
    eps = kwargs.get('eps', 1e-12)
